		...
		$(selectedRow).each(function () {
			...
			moveUserDom +=
				`<tr data-uuid="${profileId}" data-nick="${nick}" data-total="${total}">
					...
					<td>
						<div class="user-ucd">
							<strong>${numberWithCommas(total)} UCD</strong>
						</div>
					</td>
					<td>
						<i onclick="removeRow(this); calculateSelectedCount();" class="far fa-times-circle"></i>
					</td>
				</tr>`
		});
	...
	function getUser()
	{
		dataTable.DataTable({
			...
			columns: [
				...
				,{title: "닉네임",	data: "nickname",    width: "65%", 	 className: "cursor-default",
	...
	function hasDuplicateId()
	{
		...
		for (let i=0; i<selectedData.length; i++)
		{
			...
			if (movedUser.indexOf(profile_uuid) !== -1)
				result = true;
		}
